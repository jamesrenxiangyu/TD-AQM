  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      {
        bool flag = dsrHeader.GetFlag ();
        // record delivery time and service class
        if (flag)
        {
          uint32_t delay = Simulator::Now ().GetMicroSeconds () - dsrHeader.GetTxTime ().GetMicroSeconds ();
          uint32_t prio = dsrHeader.GetPriority ();
          if (prio < 2)
          {
            prio = 0;
          }
          ...
          *os << Simulator::Now().GetSeconds() << " Delay " << delay << " Class: " << prio << std::endl; 
        }
      }    
      /////
